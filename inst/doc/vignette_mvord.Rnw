...
heteroskedasticity is package \pkg{oglmx}
\citep{oglmx}. The package \pkg{ordinalNet} \citep{ordinalNetpkg}
offers tools for model selection by using an elastic net penalty. Moreover, ordinal logistic models can be fitted by the functions \code{lms()} and  \code{orm()} in package \pkg{rms} \citep{rmspkg} and the \code{MCMCoprobit()} function in package \pkg{MCMCpack} \citep{MCMCpackpkg} which uses Markov Chain Monte Carlo methods to fit ordinal probit regression models.

An overview on ordinal regression models in other statistical software packages like \proglang{Stata} \citep{Stata}, \proglang{SAS} \citep{SAS} or \proglang{SPSS} \citep{SPSS} is provided by \cite{Liu09}. These software packages include the \proglang{Stata} procedure \code{OLOGIT}, the \proglang{SAS} procedure \code{PROC LOGISTIC} and the \proglang{SPSS} procedure \code{PLUM} which perform ordinal logistic regression models. The software procedure \code{PLUM} additionally includes other link functions like probit complementary log-log, cauchit abd negative log-log. Ordinal models for multinomial data are available in the \proglang{SAS} package \code{PROC GENMOD}, while another implementation of ordinal logistic regression is available in \proglang{JMP} \citep{JMP}. In \proglang{Python} \citep{Python}, package \pkg{mord} \citep{mordpkg} implements ordinal regression methods.
...
missing observations in the outcome variable.  None of these packages
...
term. We further assume the $n$~subjects to be independent and that the error
...
category $K_j$ is the best category.  The marginal cumulative
...
mvord(formula, data, error.structure = cor_general(~ 1), link = mvprobit(),
  ...
  coef.constraints = NULL, coef.values = NULL, weights = NULL, se = TRUE,
  offset = NULL, scale = FALSE, start.values = NULL, solver = "newuoa",
  PL.lag = NULL, control = list(maxit=200000, trace = 1, kkt = FALSE))
\end{verbatim}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% # @
\begin{verbatim}
response.levels = list(Y1 = c("F", "E", "D", "C", "B", "A"),
                       Y2 = c("F", "E", "D", "C", "B", "A"),
                       Y3 = c("M", "L", "K", "J", "I", "H", "G"),
                       Y4 = c("O", "N"))
\end{verbatim}
...
covariates should be specified in such a way that multicolinearity does not arise (as if an intercept were specified). This is of primary importance when including categorical covariates, where one category will be taken as baseline by default.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Link functions}
The multivariate link functions are specified as objects of class
\code{`mvlink'}.  An object of class \code{`mvlink'} is a list with
...
link. If gradient functions passed on, these will be used in the
computation of the standard errors.  This design has been inspired by
the design of the \code{`family'} class in package \pkg{stats} and it
...
errors is applied. The normal bivariate probabilities which enter the
pairwise log-likelihood are computed with package \pkg{pbivnorm}
\citep{pbivnorm}. The multivariate probit link can be specified by:
\begin{verbatim}
link = mvprobit()
\end{verbatim}
...
of class \code{`error_struct'}. This approach is slightly different than in package \pkg{nlme},
where the error structure is defined by two classes: \code{`varFunc'} for the variance function
and \code{`corStruct'} for the correlation structure.
We also define the following subclasses for the error structures: \code{`cor_general'}
(similar to  \pkg{nlme}'s \code{`corSymm'}),  \code{`cor_equi'} (similar to \code{`corCompSymm'}),
\code{`cor_ar1'}  (similar to \code{`corAR1'}) and
\code{`cov_general'} (similar to \code{`corSymm'} with variance function \code{`varIdent'}).

The different error structures are chosen in the following way:
\subsubsection{Basic model}
\paragraph{Correlation}
For the basic model specification the following correlation structures are implemented in \pkg{mvord}:
\begin{itemize}
  \item \code{cor_general()} -- A general error structure, where the
...
\begin{verbatim}
error.structure = cor_general(formula = ~ 1)
\end{verbatim}
  \item \code{cor_equi()} -- An equicorrelation structure with
    $corr(\epsilon_{ik}, \epsilon_{il})=\rho$ is used.
\begin{verbatim}
error.structure = cor_equi(formula = ~ 1)
\end{verbatim}
\item \code{cor_ar1()} -- An autoregressive error structure of order one with
  $corr(\epsilon_{ik}, \epsilon_{il}) = \rho^{|t_{k}-t_{l}|}$ is used.
\begin{verbatim}
error.structure = cor_ar1(formula = ~ 1)
\end{verbatim}
\end{itemize}
\paragraph{Variance}
...
\begin{itemize}
  \item the estimation of $\sigma^2_j$ is only implemented in
    ...
    by:
\begin{verbatim}
error.structure = cov_general(formula = ~ 1)
\end{verbatim}
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Extending the basic model}
...
\item As in the basic model, the estimation of the heterogeneous
  ...
  can be specified by:
\begin{verbatim}
error.structure = cov_general(formula = ~ f)
\end{verbatim}
...
% # @
\begin{verbatim}
threshold.values = list(c(-4, NA, NA, NA, NA, NA), c(-4, NA, NA, NA, NA, NA),
  c(-5, NA, NA, NA, NA, NA, NA), 0)
\end{verbatim}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The constraints can be specified in this case as a vector or matrix of integers, where coefficients getting same integer value are set equal.
...
for each of the regression coefficients of the $p$~covariates, not
...
Let us assume we are dealing with seven covariates and four outcomes
(as in the example presented in Section~\ref{subsect:example1}) and
...
coef.constraints = cbind(c(1, 2, 3, 4), c(1, 1, 1, 2), c(NA, NA, NA, 1),
  c(1, 1, 1, NA), c(NA, NA, NA, NA), c(1, 2, 3, 4), c(1, 2, 3, 4))
\end{verbatim}
...
% # @
\begin{verbatim}
coef.values = cbind(c(NA, NA, NA, NA), c(NA, NA, NA, NA), c(0, 0, 0, NA),
  c(NA, NA, NA, 0), c(2, 2, 2, 2), c(NA, NA, NA, NA), c(NA, NA, NA, NA))
\end{verbatim}
...
\end{alignat*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Constraints on category-specific coefficients}
...
Let us consider the model from above and further assume outcome one, two and three are ordinal variables with four categories and outcome four has three categories. For illustration purposes we now relax the parallel regression assumption partially for covariates \code{X1} and \code{X3} in the following way:
\begin{itemize}
\item $\beta_{11, 1} = \beta_{11, 2} \neq \beta_{11,  3}$;

\item $\beta_{43, 1} \neq \beta_{43, 2}$,
\end{itemize}
where $\beta_{jk,  r}$ denotes the regression coefficient of covariate~$k$ in the linear predictor of the $r$-th cumulative probit or logit for measurement~$j$. By the first restriction,  for the first outcome two regression coefficients are employed for covariate \code{X1}: one coefficient $\beta_{11,  1} = \beta_{11, 2}$ for the first two linear predictors and one coefficient $\beta_{11,  3}$ for the third linear predictor. Covariate \code{X3} only appears in the model for the fourth  outcome. For each of the two linear predictors a different regression coefficient is estimated: $\beta_{43, 1}$ and $\beta_{43, 2}$. Note also that some of the coefficients are constant across outcomes. Moreover the coefficient of covariate \code{X5} is fixed to two by setting an offset in the model \code{formula}:
\begin{verbatim}
formula = 0 + X1 + X2 + X3 + X4 + offset(2 * X5) + X6 + X7
\end{verbatim}

The constraints are set up as a named list where the names correspond to the names of all covariates in the \code{model.matrix}. The number of rows is equal to the total number of linear predictors $\sum_j (K_j - 1)$ of the ordered responses, in the example above $3+3+3+2=11$ rows. The number of columns represents the number of parameters to be estimated for each covariate.

\begin{verbatim}
coef.constraints = list(X1 = cbind(c(1, 1, rep(0, 9)), c(0, 0, 1, rep(0, 8)),
  c(rep(0, 3), rep(1, 3), rep(0, 5)), c(rep(0, 9), rep(1, 2))),
  X2 = cbind(c(rep(1, 9), rep(0, 2)), c(rep(0, 9), rep(0, 2))),
  X3 = cbind(c(rep(0, 9), 1, 0), c(rep(0, 9), 0, 1)),
  X4 = matrix(c(rep(1, 9), rep(0, 2))), X6 = cbind(c(rep(1, 3), rep(0, 8)),
    c(rep(0, 3), rep(1, 3), rep(0, 5)), c(rep(0, 6), rep(1, 3), rep(0, 2)),
    c(rep(0, 9), rep(1, 2))), X7 = cbind(c(rep(1, 3), rep(0, 8)),
    c(rep(0, 3), rep(1, 3), rep(0, 5)), c(rep(0, 6), rep(1, 3), rep(0, 2)),
    c(rep(0, 9), rep(1, 2))))
\end{verbatim}
...
% If the parallel regression or proportional odds assumption ought to be relaxed,
% the constraint design of package \pkg{VGAM} can be employed.
% Let us consider the model from above and further assume outcome one, two and three are ordinal variables with 3 categories and outcome four has two categories. For illustration purposes we now relax the parallel regression assumption partially for covariates \code{X1} and \code{X3} in the following way:
% \begin{itemize}
% \item $\beta_{11,\text{cat}_1} = \beta_{11,\text{cat}_2} \neq \beta_{11, \text{cat}_3}$;
%
% \item $\beta_{43,\text{cat}_1} \neq \beta_{43,\text{cat}_2}$,
% \end{itemize}
% where $\beta_{jk, \text{cat}_r}$ denotes the regression coefficient for multiple measurement~$j$, covariate~$k$ and category~$r$. By the first restriction,  for the first outcome two regression coefficients are employed for covariate \code{X1}: one coefficient $\beta_{11, \text{cat}_1} = \beta_{11,\text{cat}_2}$ for the first two categories and one coefficient $\beta_{11, \text{cat}_3}$ for the third category. Covariate \code{X3} only appears the model for the fourth  outcome. For each of the two categories a different regression coefficient is estimated: $\beta_{43,\text{cat}_1}$ and $\beta_{43,\text{cat}_2}$. Note also that some of the coefficients are constant across outcomes. Moreover the coefficient of covariate \code{X5} is fixed to two by setting an offset in the model \code{formula}:
% \begin{verbatim}
% formula = 0 + X1 + X2 + X3 + X4 + offset(2 * X5) + X6 + X7
% \end{verbatim}
%
% The constraints are set up as a named list where the names correspond to the names of all covariates in the \code{model.matrix}. The number of rows is equal to the total number of classes of the ordered responses, in the example above $3+3+3+2=11$ rows. The number of columns represents the number of parameters to be estimated for each covariate.
%
% \begin{verbatim}
% coef.constraints = list(X1 = cbind(c(1, 1, rep(0, 9)), c(0, 0, 1, rep(0, 8)),
%   c(rep(0, 3), rep(1, 3), rep(0, 5)), c(rep(0, 9), rep(1, 2))),
...
matrices. Consider the following \code{data}:
<<echo = FALSE>>=
n<-3
...
data
@
where \code{X3} is a categorical variable with three categories \code{c1}, \code{c2} and \code{c3}.
...
names_constraints(formula, data)
@
This should be used when setting up the coefficient constraints. Please note that category \code{c1} for \code{X3} is taken as baseline for both cases.
%%
%
% \begin{alignat*}{7}
% \widetilde Y^{(\text{cat}_1,\text{cat}_2)}_{i1} &= \beta^{(\text{cat}_1,\text{cat}_2)}_{11} x_{i1} &&+ \beta_{12} x_{i2} &&+ \beta_{14} x_{i4} &+ 2 x_{i5} &&+ \beta_{16} x_{i6}  ,\\
% \widetilde Y^{(\text{cat}_3)}_{i1} &= \beta^{(\text{cat}_3)}_{11} x_{i1} &&+ \beta_{12} x_{i2} &&+ \beta_{14} x_{i4} &+ 2 x_{i5} &&+ \beta_{16} x_{i6},\\
% \widetilde Y_{i2} &= \beta_{21} x_{i1} &&+ \beta_{12} x_{i2} &&+  \beta_{14} x_{i4} &+ 2 x_{i5} &&+ \beta_{26} x_{i6} ,\\
% \widetilde Y^{(\text{cat}_1)}_{i3} &= \beta_{31} x_{i1} &&+ \beta_{32} x_{i2} &+ \beta^{(\text{cat}_1)}_{33} x_{i3} &&+2 x_{i5} &&+ \beta_{36} x_{i6} \\
% \widetilde Y^{(\text{cat}_2)}_{i3} &= \beta_{31} x_{i1} &&+ \beta_{32} x_{i2} &+ \beta^{(\text{cat}_2)}_{33} x_{i3} &&+2 x_{i5} &&+ \beta_{36} x_{i6}.
% \end{alignat*}
% This is, for the first outcome, two regression coefficients are employed for covariate \code{X1}: one coefficient $\beta^{(\text{cat}_1,\text{cat}_2)}_{11}$ for the first two categories and one coefficient $\beta^{(\text{cat}_3)}_{11}$ for the third category. Covariate \code{X3} only appears the model for the third  outcome. For each of the two categories a different regression coefficient is estimated: $\beta^{(\text{cat}_1)}_{33}$ and $\beta^{(\text{cat}_2)}_{33}$. Note also that some of the coefficients are constant across outcomes. Moreover the coefficient of covariate \code{X5} is fixed to two.
%
% The constraints are set up as a named list where the names correspond to the names of the covariates. The number of rows is equal to the total number of classes of the ordered responses, in the example above $3+3+2=8$ rows. The number of columns represents the number of parameters to be estimated for each covariate.
% <<eval = F>>=
% coef.constraints = list(
%   X1 = cbind(c(1, 1, rep(0, 6)), c(0, 0, 1, rep(0, 5)), c(0, 0, 0, 1, 1, 1, 0, 0),
%     c(rep(0, 6),1,1)), X2 = cbind(c(rep(1, 6), 0, 0), c(rep(0, 6), 1, 1)),
%   X3 = cbind(c(rep(0, 6), 1, 0), c(rep(0, 6), 0, 1)),
%   X4 = matrix(c(rep(1, 6), rep(0, 2))),
%   X6 = cbind(c(1,1,1,rep(0, 5)), c(0,0,0,1,1,1,0,0), c(rep(0,6), 1,1)))
% @
% The coefficient for covariate \code{X5} fixed to two can be specified in the formula as:
% \begin{verbatim}
% formula = 0 + X1 + X2 + X3 + X4 + offset(2 * X5) + X6
% \end{verbatim}
%
% For more details we refer the reader to the documentation of the \pkg{VGAM} package.
...
%
\subsubsection[scale]{\code{scale}}
If \code{scale = TRUE}, the continuous covariates are standardized by substracting the mean and dividing by the standard deviation. This operation is performed for each repeated measurement before fitting.
%
...
log-likelihood. These are \code{"Nelder-Mead", "BFGS", "CG",
  ...
  "nmkb", "hjkb", "Rcgmin"} and \code{"Rvmmin"}. The default is the \code{"newuoa"} solver.
...
start.values = list(theta = list(c(-3, -1, 0, 0.5, 2.5),
  c(-3, -1, 0, 0.5, 2, 3.5), c(0)), beta = list(c(0.05, -0.05, -0.8, 1, 0.2),
    c(-0.5, 0.2), c(-0.3, 0.3), c(0.5, -1.1, 0.7, 0.3, -1.2)))
\end{verbatim}
...
\code{contrasts} can be specified by a named list as in the argument \code{contrasts.arg} in \code{model.matrix.default()}.
...
the threshold coefficients and a function \code{get_error_struct()} to
...
provides the number of subjects. In addition, a \code{predict()}
function, a \code{marginal.predict()} function and a \code{get.prob()}
function are available to predict probabilities, cumulative
probabilities, categories and linear predictors for the fitted model.
Other standard methods such as \code{terms()}, \code{model.matrix()}
and \code{fitted()} are also available. Functions \code{AIC()} and
\code{BIC()} can be used to extract the information criteria CLAIC and
CLBIC.
%
\subsection{Output}
...
\code{error.struct} & an object of class \code{`error\_struct'}\\
...
mvord2(formula, data, error.structure = cor_general(~ 1), link = mvprobit()
  ...
  threshold.values = NULL, weights = NULL, se = TRUE, offset = NULL,
  scale = FALSE, start.values = NULL, solver = "newuoa", PL.lag = NULL,
...
Each outcome must be an ordered factor.
...
The first example presents a multivariate ordinal regression model with logit link and a general correlation error structure (\code{cor_general(~ 1)}).
...
@
We include 7 financial ratios as covariates in a model without intercept by the \code{formula}:
\begin{verbatim}
...
\begin{verbatim}
coef.values = cbind(c(NA, 0, NA, 0), c(0, 0, 0, NA), c(NA, NA, NA, 0),
...
@
If the user fixes thresholds to specific values, the $z$~values and the corresponding $p$~values as well as the significance codes are set to \code{NA}. Another option to display the results is the function \code{print()}:
<<>>=
...
coef(res_cor_logit)
@
The error structure for firm with \code{firm_id} 11 is displayed by the function \code{get_error_struct()}:
<<>>=
...
@
%% first discuss input
We include the 7 financial ratios as covariates in a model without intercept by the \code{formula}:
\begin{verbatim}
formula = rating ~ 0 + ICR + LR + LEV1 + LEV2 + PR + lRSIZE + lSYSR
\end{verbatim}
...
coef(res_AR1_probit)
@
The error structure is displayed by the function \code{get_error_struct()}:
<<>>=
...
on threshold as well as regression coefficients. Two different data formats
can be used either by applying \code{mvord()} or \code{mvord2()} to estimate the model parameters.
