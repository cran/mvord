...
\citep{VGAMpkg}. Another package which accounts for
...
covariates should be specified in such a way that multicolinearity does not arises (as if an intercept were specified). This is of primary importance when including categorical covariates, where one category will be taken as baseline by default.
