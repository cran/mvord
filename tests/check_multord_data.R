...
data.multord <- mvord:::multord.data(data, index, y.names, x.names, y.levels = NULL, response.names)
tmp <- cbind.data.frame( A = ordered(c(1,NA,NA)),
                         "1" = ordered(c(3,4,7)),
                         "3" = ordered(c(2,6,9)),
                         "2" = ordered(c(NA,5,8)))
mvord:::check(identical(data.multord$y, tmp))
...
mvord:::check(identical(data.multord$x, tmp))
...
z <- mvord:::multord.data(data, index, y.names, x.names, y.levels, response.names)
...
#should not work
mvord:::check(!is.null(attr(try(mvord:::multord.data(data, index, y.names, x.names, y.levels, response.names), silent = TRUE), "condition")))
